                    ...
                    >
                      ...
                      {msg.role === 'bot' && msg.id !== 'welcome' ? (
                        <div className="space-y-3">
                          <div className="border border-border-custom rounded-lg overflow-hidden">
                            <button onClick={() => setExpandedOverview(!expandedOverview)} className="w-full flex items-center justify-between p-2.5 bg-background-secondary hover:bg-border-custom/25 transition-all text-xs font-semibold cursor-pointer">
                              <span className="flex items-center space-x-1.5"><Info size={13} className="text-text-secondary" /><span>1. {t('overview')}</span></span>
                              {expandedOverview ? <ChevronDown size={13} /> : <ChevronRight size={13} />}
                            </button>
                            {expandedOverview && <div className="p-3 text-xs text-text-secondary leading-relaxed bg-background-primary border-t border-border-custom">Detailed expression evaluation of: <strong className="text-text-primary font-mono">{currentQuery}</strong>.</div>}
                          </div>
                          <div className="border border-border-custom rounded-lg overflow-hidden">
                            <button onClick={() => setExpandedSteps(!expandedSteps)} className="w-full flex items-center justify-between p-2.5 bg-background-secondary hover:bg-border-custom/25 transition-all text-xs font-semibold cursor-pointer">
                          ...
                          <div className="border border-border-custom rounded-lg overflow-hidden">
                            <button onClick={() => setExpandedFinal(!expandedFinal)} className="w-full flex items-center justify-between p-2.5 bg-background-secondary hover:bg-border-custom/25 transition-all text-xs font-semibold cursor-pointer">
                              <span className="flex items-center space-x-1.5"><Check size={13} className="text-text-secondary" /><span>3. {t('finalAnswer')}</span></span>
                              {expandedFinal ? <ChevronDown size={13} /> : <ChevronRight size={13} />}
                            </button>
                            {expandedFinal && <div className="p-3 bg-background-primary border-t border-border-custom"><div className="font-mono text-xs text-text-primary leading-relaxed" dangerouslySetInnerHTML={{ __html: msg.content }}></div></div>}
                          </div>
                          <button onClick={() => addBookmarkItem('solution', `Solution: ${msg.rawQuery}`, msg.rawQuery)} className="text-[10px] font-mono text-text-secondary hover:text-text-primary flex items-center space-x-1 pt-1.5"><Bookmark size={11} /><span>{t('saveBookmark')}</span></button>
                        </div>
                      ) : (
                        <div className="font-mono text-xs leading-relaxed text-text-primary animate-fade-in" dangerouslySetInnerHTML={{ __html: msg.content }}></div>
                      )}
                    </div>
                  </div>
                ))}
                {loading && (
                  <div className="flex justify-start"><div className="bg-background-secondary border border-border-custom w-full max-w-sm p-4 rounded-xl rounded-tl-none space-y-3 shadow-xs">
                    <div className="flex justify-between items-center text-[9px] font-mono text-text-secondary uppercase pb-1 border-b border-border-custom"><span>Executing</span><span className="animate-pulse font-bold" style={{ color: 'var(--accent)' }}>COMPILING...</span></div>
                    <div className="space-y-2 animate-pulse"><div className="h-3 bg-border-custom rounded w-1/3"></div><div className="h-2.5 bg-border-custom rounded w-3/4"></div><div className="h-2 bg-border-custom rounded w-5/6"></div></div>
                  </div></div>
                )}
                <div ref={messagesEndRef} />
              </div>
              <div className="p-4 border-t border-border-custom bg-background-secondary space-y-3">
                <div className="flex flex-wrap gap-1.5 justify-center md:justify-start">
                  {quickChips.map((chip, idx) => (
                    <button key={idx} onClick={() => setInput(chip + " ")} className="px-2.5 py-1 rounded-lg bg-background-primary hover:bg-border-custom/40 border border-border-custom transition-all text-[11px] font-medium text-text-secondary hover:text-text-primary cursor-pointer">{chip}</button>
                  ))}
                </div>
                <form onSubmit={handleSend} className="flex gap-2 items-center">
                  <input type="text" placeholder={t('askPlaceholder')} value={input} onChange={(e) => setInput(e.target.value)} disabled={loading} className="flex-grow px-3 py-2.5 rounded-xl border border-border-custom bg-background-primary text-xs" style={{ '--tw-ring-color': 'var(--accent)' }} />
                  <button type="submit" disabled={loading || !input.trim()} className="px-4 py-2.5 rounded-xl font-bold transition-all disabled:opacity-50 cursor-pointer text-xs" style={{ backgroundColor: 'var(--accent)', color: 'var(--accent-text)' }}>{t('solve')}</button>
                </form>
              </div>
            </div>
          )}
